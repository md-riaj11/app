...
class SearchHistoryList extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    final _history = ref.watch(searchHistoryController);
    return Column(
      children: [
        Padding(
                ...
                data: (data) => data.value.isNotEmpty
                    ? AnimatedList(
                        ...
                        itemBuilder: (context, index, animation) {
                          final _current = data.value[index];
                          return SearchTile(
                            current: _current,
                            animation: animation,
                            key: ValueKey(_current.query),
                            onDelete: () {
                              ref
                                  .read(searchHistoryController.notifier)
                                  .deleteEntry(_current);

                              final _currentItem = data.value[index];
                    ...
                    : Column(
                        children: [
                          Padding(
                            padding: const EdgeInsets.all(AppDefaults.padding),
                            child: Image.asset(AppImages.illustrationUpdated),
                          ),
                          AppSizedBox.h16,
                          Text(
                            'No history here',
