...
class SavedPostNotifier extends StateNotifier<AsyncValue<List<ArticleModel>>> {
  ...
  Future<List<ArticleModel>> getAllSavedPost() async {
    ...
    final _allPosts = await _repo.getsavedPosts();

    // Fix for hero animations
    final _newList =
        _allPosts.map((e) => e.copyWith(heroTag: e.link + 'saved')).toList();
  ...
  removePostFromSavedAnimated(
    ...
    final _newList = state.value?.where((element) => element.id != id).toList();
    if (_newList != null) {
      state = AsyncData(_newList);
  ...
  addPostToSaved(ArticleModel article) async {
    isSavingPost = true;
    final _currentList = state.value;
    state = const AsyncLoading();
    final _thePost = article.copyWith(heroTag: article.link + 'saved');
    state = AsyncValue.data([..._currentList!, _thePost]);
